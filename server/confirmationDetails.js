...
const { fetchMarketPrice } = require('./market');
...
function generateConfirmationQueryParamsLocal(account, tag) {
  const params = generateConfirmationQueryParams(account, tag);
  return params;
}
  ...
  for (const [, item] of byKey) {
    try {
      ...
      if (item.appid && item.marketHashName) {
        const price = await fetchMarketPrice(item.appid, item.marketHashName, 1);
        if (price) {
          item.price = {
...
function parseTradeHtml(html) {
  ...
  function parseItemList(root, targetArray, sideLabel) {
    ...
    items.each((_, el) => {
      ...
      const assetid = node.attr('data-assetid') || node.attr('data-id') || null;
...
async function fetchMobileConfDetails(account, payload) {
  await alignTime('Details');
  const cookie = getSessionCookieHeader(account);
  const qsBase = generateConfirmationQueryParamsLocal(account, 'details');
